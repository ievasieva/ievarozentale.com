...
```{r findingfactors, warning=FALSE, echo=FALSE, message=FALSE}
#Finding the number of factors
par <- fa.parallel(values) #analysis suggests 3 factors 
```

```{r EFA graph, warning=FALSE, echo=FALSE}
...
```{r EFA loadings, warning=FALSE, echo=FALSE}
...
These are the Gris (2001) validity scores: 
```{r iclust, echo=F, warning=F}
iclust(values)
```


```{r iclust2, echo=F, warning=F}
fa.diagram(ICLUST(values,2,title="Two cluster solution"),main="Input from ICLUST")
```


```{r omega, echo=FALSE, warning=FALSE}
omega(values)
```
